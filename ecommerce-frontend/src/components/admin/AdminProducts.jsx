      ...
      {showForm && (
        <form className="admin-form" onSubmit={handleSubmit}>
          ...
          <div className="form-grid-modern" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '16px', marginBottom: '16px' }}>
            ...
            <div className="input-group-modern">
               <label style={{ fontSize: '0.8rem', color: 'var(--color-text-muted)' }}>Price (TL)</label>
               <input 
                type="number" 
                step="0.01" 
                placeholder="1000.00" 
          ...
          <tbody>
            {products.map(p => (
              <tr key={p.id}>
                ...
                <td>{p.price.toLocaleString('tr-TR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} TL</td>
                <td>
                ...
                <td className="admin-table-actions">
                  <button className="edit-mini" onClick={() => handleEdit(p)}>✏️</button>
                  <button className="delete-mini" onClick={() => handleDelete(p.id)}>🗑️</button>
                </td>
              </tr>
            ))}
          </tbody>
